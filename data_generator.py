...
def save_random_traingle(index, path):
    ...
    with open('{}train/{}.txt'.format(path, index), 'w+') as f:
    ...
    img.save('{}obj/{}.jpg'.format(path, index))

    with open('{}train.txt'.format(path), 'a') as f:
        f.write('{}obj/{}.jpg\n'.format(path, index))


pathlib.Path('{}train'.format(PATH)).mkdir(parents=True, exist_ok=True)
pathlib.Path('{}obj'.format(PATH)).mkdir(parents=True, exist_ok=True)


for i in range(SET_SIZE):
    save_random_traingle(i, PATH)
